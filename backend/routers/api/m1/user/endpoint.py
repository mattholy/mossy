...
class Avatar(BaseModel):
    ...
    file_type: Optional[str] = Field(
        "", description="MIME type of the avatar file")
...
class Header(BaseModel):
    ...
    file_type: Optional[str] = Field(
        "", description="MIME type of the header file")
...
class UserProfile(BaseModel):
    display_name: str = Field(
        "John Doe", description="Display name of the user")
...
async def fetch_user_profile(user_session: UserSession = Depends(get_current_user_session), db: AsyncSession = Depends(get_db)):
    fedi_user_query = select(FediAccounts).where(
...
async def fetch_user_profile(profile_data: UserProfile, user_session: UserSession = Depends(get_current_user_session), db: AsyncSession = Depends(get_db)):
